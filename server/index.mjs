...
if (!jwtSecret) throw new Error('Falta JWT_SECRET en las variables de entorno')
...
app.post('/api/auth/login', async (request, response) => {
  ...
  if (!usuario || !password) return response.status(400).json({ error: 'Usuario y contraseña son requeridos' })

  const result = await pool.query(
    'SELECT id_usuario, usuario, password_hash FROM usuarios WHERE usuario = $1 AND activo = TRUE',
    [usuario],
  )
  ...
  const token = jwt.sign({ sub: user.id_usuario, usuario: user.usuario }, jwtSecret, { expiresIn: '8h' })
  response.json({ token, usuario: user.usuario })
})
...
app.put('/api/clientes/:id', requireAuth, async (request, response) => {
  const { fechaConexion, abonado, plan, valor, ip, ultimoMesPagado, estado } = request.body
  const result = await pool.query(
    `UPDATE clientes
...
app.patch('/api/clientes/:id/baja', requireAuth, async (request, response) => {
  const result = await pool.query(
    `UPDATE clientes
...
app.post('/api/clientes/:id/pagos', requireAuth, async (request, response) => {
